...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="z-50 relative flex bg-[#0A1828] text-[#FFD700] font-bold px-8 py-4 items-center justify-between border-b-4 border-[#FFD700]">
        ...
        <div className="navlinks block md:flex items-center gap-4">
            ...
            {links.map((item, i) =>(
              <div className="flex items-center justify-center" key={i}>
                {item.title === "Profile" || item.title === "Admin Profile" ?
                  <Link to={item.link} className="text-[#FFD700] bg-gradient-to-r from-green-500 via-green-600 to-green-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                    {item.title}
                  </Link> :
          ...
          <button className='block md:hidden text-[#FFD700] text-2xl hover:text-red-500' onClick={() => (MobileNav === "hidden" ? setMobileNav("block") : setMobileNav("hidden"))}>
            <FaGripLines />
          </button>
        </div>
      </nav> 
      <div className={`${MobileNav} bg-[#0A1828] h-screen absolute top-0 left-0 w-full z-40 flex flex-col items-center justify-center`}>
        {links.map((item, i) =>(
          <Link to={item.link} className="mb-8 text-[#FFD700] text-4xl font-semibold hover:text-green-500 transition-all duration-300"
            key={i}
            onClick={() =>
              MobileNav === "hidden"
                ? setMobileNav("block")
